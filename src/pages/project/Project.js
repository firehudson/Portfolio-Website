...
export const Project = ({ posts, featured }) => {
  ...
  const postsHeader = (
    <header className={styles.header}>
      <Heading className={styles.heading} level={5} as="h1">
        <DecoderText text="Project Samples" />
      </Heading>
    </header>
  );
    ...
    <div className={styles.list}>
      ...
      {posts.map(({ slug, ...post }, index) => {
        if (
          ...
          return <ProjectPost key={slug} slug={slug} index={index} {...post} />;
      })}
      {Array(2)
        .fill()
        .map((skeleton, index) => (
  ...
  const featuredPost = <ProjectPost {...featured} />;

  return (
    <article className={styles.Project}>
      <Meta
        title="Project"
        description="A collection of technical design and development Project. May contain incoherent ramblings."
      />
